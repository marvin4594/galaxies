...
def halo_mag(simstars, band='v', addage=0.0, lumfile = "cmdlum.npz"):

    if (len(simstars)==0): return np.inf
    ...
    try:
        if any(s['massform'] < 1e-100 for s in simstars):
...
def center_avd(sim):
    ...
    if (len(sim.s[fr]) < 5 and len(sim.d[fr]) < 5 and len(sim.g[fr]) < 5):
        if (len(sim.s)>=5):
            cen_size = h_smooth(sim.s, N=5)
        elif(len(sim.d)>=5):
            cen_size = h_smooth(sim.d, N=5)
        else:
            cen_size = h_smooth(sim.g, N=5)                   
